...
function check_req_fields()
{
    ...
    for(i=0; i<index; i++)
    {
       ...
       if (document.getElementById(ln).value.length != 0)
       {
           ...
           {
             errmsg += 'Please enter CRN at Line' + lnv + '\n';
           }
...
function Setcrn_assyrev(CIM,fieldname,lnnum,pqty)
{
   ...
   var id4="bom"+ fieldname;
   var text4= document.getElementById(id4);
   text4.value=CIM[2];

   var id6="bom_iss"+ fieldname;
   ...
   var id8="pcrn"+ fieldname;
   var text8= document.getElementById(id8);
   text8.value=CIM[0];
   fieldname++;
   //alert(CIM[4]+"m--a---i---n");
   var CIMarr = CIM[4].split("%");
   //alert(CIMarr[0]);
  // alert(fieldname+"----"+CIMarr.length);
  // getLIs(CIM[0],CIM[2]);
   for(i=0;i<=(CIMarr.length-1);i++)
   {
     //alert(fieldname+"----"+i+"-----"+CIMarr[i]+"----"+i);
     if(CIMarr[i]!='')
     {   j=0; m=1; n=0; z=1; k=0;
       cimqtyarr=CIMarr[i].split("*");
       //alert(cimqtyarr[j]+"---------");
       //alert(cimqtyarr[m]+"---nxt------");
       cim_qtyarr=cimqtyarr[m].split(":");
       //alert(cim_qtyarr[1]+"==================");
       cim_partarr=cim_qtyarr[z].split("^");
...
inp2.size="10";
inp2.style.backgroundColor = "#DDDDDD";
inp2.value=cimqtyarr[j];
inp2.readOnly = true;
//var img1 = document.createElement("img");
//img1.setAttribute("src","images/getcim.gif");
//img1.setAttribute("alt","GetCIM");
//img1.onclick = function(){GetCrn4asyrevli(y);};
cell2.appendChild(inp2);
//cell2.appendChild(img1);
...
inp17.size="8";
inp17.readOnly=true;
inp17.style.backgroundColor = "#DDDDDD";
inp17.value=cim_partarr[k+2];
cell17.appendChild(inp17);
