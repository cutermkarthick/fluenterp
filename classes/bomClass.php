    ...
    function addBOM()
    {
        ...
        $siteid= "'" . $_SESSION['siteid'] . "'";
          ...
          // echo $sql;exit;
           ...
           {
              $sql = "INSERT INTO
			bom (recnum, bomnum, bom_issue, crn, assy_partnum, title, issue,cos_no,cos_iss,drg_no,bom_revnum,eng_app,eng_app_by,eng_app_date,status,create_date,partiss,siteid)
                         VALUES
			($objid, $bomnum, $bom_issue, $crn, $assy_part,$title, $issue,$cos_no,$cos_iss,$drg_no,$bomrevnum,$eng_app,$eng_app_by,$eng_app_date,$status,now(),$partiss,$siteid)";
            // echo $sql;exit;
              $result = mysql_query($sql);
...
//function to get only BOMs where bomnum=partnum
   ...
   // echo $result;
    ...
    function getBOM_summary($cond,$argoffset,$arglimit)
    {
       ...
       $sql = "select b.recnum,b.bomnum,b.bom_issue,b.crn,b.assy_partnum, 
                  ...
                  from bom b where $wcond and  $siteval
    ...
    function getBOMcount($cond,$argoffset, $arglimit)
    {
        ...
        $sql = "select count(*) as numrows from bom b where $wcond  and $siteval
               limit $offset, $limit";
        //echo $sql;
        $newlogin = new userlogin;
        $newlogin->dbconnect();
  ...
  function getassy_crn()
    ...
    function getbom_assyWo_partDetails($bomnum,$bomiss)
    {
       ...
       $siteid = $_SESSION['siteid'];
       $siteval = "b.siteid = '".$siteid."'";
       $sql = " select  bom_subassy.item_no as itn,bom_subassy.partnum,bom_subassy.partname, bom_subassy.partiss,'Assembly',
                   ...
                   from bom b,bom_subassy_items bom_subassy
                    where b.bomnum = '$bomnum'  and b.bom_revnum='$bomiss'
                         and bom_subassy.link2bom=b.recnum  and b.status ='Active' and $siteval
                         UNION
          select  bom_mfg.item_no as itn,bom_mfg.partnum,bom_mfg.partname, bom_mfg.partiss,'Non Assembly',
                       bom_mfg.crn,bom_mfg.qpa,bom_mfg.crn_type,''
                   from bom b,bom_mfg_items bom_mfg
                   where b.bomnum = '$bomnum' and b.bom_revnum='$bomiss'
                         and bom_mfg.link2bom=b.recnum and b.status ='Active' and $siteval
                UNION
               select  bom_bo.item_no as itn,bom_bo.partnum,bom_bo.descr, bom_bo.partiss,'Bought Out','',bom_bo.qpa,'',''
                   ...
                   where b.bomnum = '$bomnum' and b.bom_revnum='$bomiss'
                         and bom_bo.link2bom=b.recnum and b.status ='Active' and $siteval
                UNION
               select  bom_co.item_no as itn,'',bom_co.descr,'','Consummables','',bom_co.qpa ,'',bom_co.spec
                   from bom b,bom_consume bom_co
                    where b.bomnum = '$bomnum' and b.bom_revnum='$bomiss'
                         and bom_co.link2bom=b.recnum and b.status ='Active' and $siteval
                UNION
                 select  bom_treat.item_no as itn,bom_treat.partnum,bom_treat.partname, bom_treat.partiss,'Non Assembly',
                       bom_treat.crn,bom_treat.qpa,bom_treat.crn_type,''
                   from bom b,bom_treated_items bom_treat
                   where b.bomnum = '$bomnum' and b.bom_revnum='$bomiss'
                         and bom_treat.link2bom=b.recnum and b.status ='Active' and $siteval";
       // echo "$sql";exit;
       $result = mysql_query($sql);
       return $result;
    }
    ...
    function getbom_assyOper($bomnum,$bomiss)
    {
       ...
       $sql = "select bom_op.opn_num,bom_op.stn,bom_op.oper_desc,bom_op.signoff,bom_op.remarks
               from bom b,bom_op_desc bom_op
                where b.bomnum='$bomnum'  and b.bom_revnum='$bomiss'
                and b.recnum = bom_op.link2bom   and b.`status` ='Active' and $siteval
            ...
            function getbom4assyWopartDetails($bomnum,$crn_num,$bomiss)
    {
       ...
       bom_mfg.crn,bom_mfg.qpa,bom_mfg.crn_type,''
                   from bom b,bom_mfg_items bom_mfg
                   where bom_mfg.link2bom=b.recnum and  b.crn='$crn_num'  and b.status ='Active' and b.bom_revnum='$bomiss' and $siteval
                UNION
               select  bom_bo.item_no as itn,bom_bo.partnum,bom_bo.descr, bom_bo.partiss,'Bought Out','',bom_bo.qpa,'',''
                   from bom b,bom_bought_items bom_bo
                   where bom_bo.link2bom=b.recnum and b.crn='$crn_num' and b.status ='Active' and b.bom_revnum='$bomiss' and $siteval
                UNION
               select  bom_co.item_no as itn,'',bom_co.descr,'','Consume','',bom_co.qpa,'',bom_co.spec
                   from bom b,bom_consume bom_co
                    where bom_co.link2bom=b.recnum and b.crn='$crn_num' and b.status ='Active' and b.bom_revnum='$bomiss' and $siteval
               order by itn
               UNION
                 select  bom_treat.item_no as itn,bom_treat.partnum,bom_treat.partname, bom_treat.partiss,'Treated',
                       bom_treat.crn,bom_treat.qpa,bom_treat.crn_type,''
                   from bom b,bom_treated_items bom_treat
                   where b.bomnum = '$bomnum' and b.bom_revnum='$bomiss'
                         and bom_treat.link2bom=b.recnum and b.status ='Active' and $siteval";
...
//echo $sql;
     ...
     function addbomnotes($bomrecnum,$bomnotes)
  {
    ...
    function getAllBOMs()
    {
        ...
        $siteid = $_SESSION['siteid'];
        $siteval = "siteid = '".$siteid."'";
        $sql = "select recnum, bomnum, bom_issue, crn, assy_partnum, title, issue,cos_no,cos_iss,drg_no,bom_revnum,eng_app,eng_app_by,eng_app_date,status,create_date,partiss FROM bom where $siteval";
        // echo $sql;
        $result = mysql_query($sql);
        //if(!$result) die("Access to BOM failed...Please report to SysAdmin. " . mysql_error());
        // echo '----'.$result;
