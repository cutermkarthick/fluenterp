...
class assywo {
  ...
  function getac_ship_date() {
           ...
           if (!(mysql_fetch_row($result)))
           {
              $sql = "INSERT INTO
		  	  assy_wo (recnum,assy_wonum, assydate, woqty, crn, link2cust, ponum,poqty,assypartnum,assypartiss,
                      ...
                      type_remarks,assy_type,rework_grn,siteid)
                         VALUES
                     ...
                     $mps_rev,$link2mps,$fai_type,$type_remarks,$assy_type,$rework_grn,$siteid)";
              //echo $sql;
              $result = mysql_query($sql);
    ...
    function getassyWo_summary($cond,$argoffset,$arglimit)
    {
        ...
        if($usertype == 'EMPL')
       {
                  $sql = "select a.recnum,a.assy_wonum,a.assydate,a.assyqty,a.crn,a.bomnum,a.cust_po_line_num,
                      ...
                      from assy_wo a,company c
                           where $wcond 
                                  and c.recnum=a.link2cust and $siteval
                                  limit $offset, $limit";
        }else
        {
            ...
            $sql = "select a.recnum,
                            ...
                            from assy_wo a,
                                 ...
                                 contact cont
                            where $wcond and 
                                        c.recnum=a.link2cust and 
                                        cont.recnum = u.user2contact and cont.contact2company = c.recnum
                                        and u.userid = '$userid' limit $offset, $limit";
      ...
      $siteid = $_SESSION['siteid'];
        $siteval = "a.siteid = '".$siteid."'";
             $sql = "select a.crn,bomnum,a.bomiss,a.assypartnum,a.attachments, a.drgno,a.drgiss,a.cosno,
                     ...
                     where a.assy_wonum=g.wo_ref  and g.grn_classif='Assy' and $siteval";
		 }
       // echo "$sql";  exit;
       $result = mysql_query($sql);
       return $result;
   ...
   function getwos4assywo($crn,$type,$crn_type)
   {
...
$qty_wo=($this->qty_wo)?$this->qty_wo:0;
  ...
  if($type == 'Non Assembly')
  {
    if($crn_type == 'Untreated')
    {
$sql = " select  w.wonum,w.qty,w.recnum,w.crn_num,
             w.comp_qty - w.dispatch_qty -w.assy_qty,
             w.assy_qty,w.dispatch_qty,w.treatment
           from work_order w 
           where w.treatment='Untreated' and
            w.`condition` ='Closed' and
            w.crn_num='$crn' and
            w.comp_qty - (w.dispatch_qty+w.assy_qty) >0  and
            w.comp_qty > 0 and $siteval";
// echo $sql;
  }
  else 
  {

  $sql = " select  dn.wonum,dn.qty,w.recnum,dn.crn,
             dn.comp_qty - dli.disp_qty -dli.assy_qty,
             dli.assy_qty,dli.disp_qty,'',w.treatment,dn.recnum
              from  work_order w ,
                     delivery_note dn,
                     delivery_note_li dli
              where  dn.wonum = w.wonum and
                     dn.recnum = dli.link2delivery and
                     w.treatment='Treated' and
                     w.`condition` ='Closed' and
                     w.crn_num='$crn' and
                     dn.comp_qty - (dli.disp_qty+dli.assy_qty)>0  and
                     dn.comp_qty > 0 and $siteval";
   ...
   function getwos4assycrnwo($crn)
   {
     ...
     $sql = "select w.assy_wonum,w.assyqty,w.recnum,w.crn
                    from assy_wo w
                         where
                               w.`status`='Open' and
                               w.crn='$crn' and w.comp_qty - (w.dispatch_qty +w.kit_qty) > 0  and
                    w.comp_qty > 0 and $siteval ";
                              echo $sql;exit;
     $result = mysql_query($sql);
     ...
     function getassywoqty4grn() {
       ...
       function getallgrns4assy($partnum)
      {
        ...
        $sql1 = "select  g.recnum, g.grnnum,g.batch_num, sum(gli.qty_to_make) as qtm,
                     ...
                     (g.grntype='Boughtout') and $siteval
                       group by g.grnnum";
        // echo "$sql1<br>";
		...
		function getallgrns4cons($type)
      {
        ...
        $sql1 = "select  g.recnum, g.grnnum,g.batch_num, sum(gli.qty_to_make) as qtm,
					 ...
					 (DATEDIFF(gli.expdate,'$to_date')>0 || gli.expdate ='0000-00-00' || g.status is null) and $siteval
                       group by g.grnnum"; 
                       // echo $sql1;
        $result1 = mysql_query($sql1);
...
function getgrnli($grnrecnum) {
    ...
    function getdisputd4treat($wonum,$supwo)
   {
       ...
       $newlogin->dbconnect();
         ...
         $sql ="select w.recnum,w.wonum,w.partnum,w.grnnum,w.po_num, w.po_qty,
                 w.po_date,w.qty,w.comp_qty,sum(dli.dispatch_qty),c.name, wps.acc, '',(w.dispatch_qty+w.assy_qty),w.assy_qty ,''
                 from wo_part_status wps, company c, work_order w, dispatch_line_items dli
                 where w.wonum = dli.wonum and w.wo2customer = c.recnum and
                 w.wonum = '$wonum' and wps.link2wo = w.recnum  and
                 wps.stage = 'fi' and w.treatment = 'Untreated' group by dli.supplier_wonum
                  UNION 
                 select w.recnum,w.wonum,w.partnum,w.grnnum,w.po_num,
                  w.po_qty, w.po_date,w.qty,w.comp_qty,
                  w.dispatch_qty,c.name, w.comp_qty, '',
                  (w.dispatch_qty+w.assy_qty),w.assy_qty,''
                  from   company c, work_order w
                  where w.wo2customer = c.recnum and
                  w.wonum = '$wonum' and
                  (w.treatment = 'Untreated') and
                  w.comp_qty > 0
                  UNION
                 select w.recnum,w.wonum,w.partnum,w.grnnum,w.po_num, w.po_qty,
                  w.po_date,w.qty,w.comp_qty,w.dispatch_qty, c.name, wps.acc, sum(ali.qty_wo-ali.qty_ret),
                  (w.dispatch_qty+w.assy_qty),w.assy_qty,'' from wo_part_status wps, company c, work_order w, assywo_li ali where w.wonum = ali.grn and 
                   substr(ali.remarks, instr(ali.remarks,'WO# ')+4, (instr(ali.remarks,'DN#')-instr(ali.remarks,'WO#'))-4) = '$supwo' 
                  and w.wo2customer = c.recnum and w.wonum = '$wonum' 
                  and wps.link2wo = w.recnum and  wps.stage = '' and (w.treatment = 'Treated'   || w.treatment ='Untreated') group by ali.grn 
                  UNION 
                  select w.recnum,w.wonum,w.partnum,w.grnnum,w.po_num, w.po_qty,
                  w.po_date,w.qty,w.comp_qty,sum(dli.disp_qty), c.name, '', sum(dli.qty_acc),
                  sum(dli.disp_qty+dli.assy_qty),IFNULL(sum(dli.assy_qty),0),dn.recnum from company c, work_order w, delivery_note_li dli, delivery_note dn where 
                     dn.recnum = dli.link2delivery and dn.wonum = w.wonum 
                    and w.wo2customer = c.recnum and w.wonum = '$wonum' 
                    and (w.treatment = 'Treated') group by dn.wonum"; 
