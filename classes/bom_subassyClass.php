...
class bomli_subassy_items {
    ...
    function addLi() {
        ...
        $sql = "select nxtnum from seqnum where tablename = 'bom_subassy_items' for update";
        $result = mysql_query($sql);
        if(!$result) {
           $sql = "rollback";
           $result = mysql_query($sql);
        ...
        $sql = "INSERT INTO bom_subassy_items (recnum, line_num, item_no, partnum, crn, partname,
        ...
        $sql = "update seqnum set nxtnum = $objid where tablename = 'bom_subassy_items'";
        $result = mysql_query($sql);
    ...
    function updateLI($recnum) {
     ...
     function get_subassy_li($inpbomrecnum) {
...
function getLI4Initial($bomrecnum)
{
    ...
    function getcrnDetails()
    {
     ...
     function getcrnDetails4bommfg($mtype)
    {
       $newlogin = new userlogin;
       $newlogin->dbconnect();
       if($mtype=='Manufacture')
       {
         $m_type= "(m.type = 'Manufacture' || m.type is NULL || m.type = '')" ;
       }else
       {
        $m_type= "(m.type='Assembly') ";
       }
       $sql = "select m.recnum,m.partname,m.wonum,m.customer,m.partnum,
                      m.RM_by_CIM,m.project,m.attachments,
                      m.RM_by_customer,m.CIM_refnum as cim,m.drg_issue,
     ...
     function deleteLI($inplirecnum) {
	    ...
	    function getcrnDetails4bommfg_edit($crn,$mtreat)
    {
      ...
      // echo $mtreat;
       ...
       if($mtreat=='Assembly')
       {
        ...
        $m_type= "(m.treat='Assembly') and m.CIM_refnum='$crn'";
       }

       
           if($mtreat == 'Treated')
           {
        $m_type= "(m.treat='Treated') and m.CIM_refnum='$crn'";
       }
	   else
       {
         $m_type= "(m.treat = 'Untreated') and m.CIM_refnum='$crn'";
       }
