        ...
        if (!(mysql_fetch_row($result))) {
           $sql = "INSERT INTO work_flow_config
                         (recnum, stage, dept, type, doc_type,
                          ...
                          create_date, cust_status_disp,est_time,act_status,est_cost,dependency,siteid, secondary_responsibility, process, when_process)
                        VALUES
                             ($objid, $stage, $dept, $type, $doctype,
                              $status, $emaillist, $apprtype, $apprby,
                              $allowcustdisp, $allowprinting, $allowreporting,
                              $crdate, $custstatusdisp,$est_time,$act_status,$est_cost,$dependency,$siteid,$sec_respose,$process,$when_process
                   )";
      // echo   $sql;exit;
           $result = mysql_query($sql);
