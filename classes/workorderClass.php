...
class workOrder {
 ...
 function getWorkOrders($username,$userid,$siteid,$usertype,$userrole,$cond,$sort1,$sort2,$argoffset,$arglimit) {
        ...
        if ($usertype == 'CUST') {
           if ($userrole == 'SU')
           {
             ...
             $sql = "select w.wonum, w.wotype,name,             w.po_num,w.po_date, w.condition,
                          w.condition, w.wo2type,w.create_date, '','', w.recnum, w.descr, u.initials, date_format(w.sch_due_date,'%d %b %y') as sch_due_date, date_format(w.actual_ship_date,'%d %b %y') as actual_ship_date, date_format(w.book_date,'%d %b %y') as book_date, date_format(w.revised_ship_date,'%d %b %y') as revised_ship_date, reorder,w.filename1, w.filename2,w.filename3, w.filename4, w.qty, w.po_qty, grnnum,w.woclassif,w.original_qty, w.fai,w.treatment,w.book_date,w.rm_spec,w.rm_type
                       from work_order w,                
        ...
        else if ($usertype == 'EMPL')
         {
           ...
           if ($userrole == 'SU' || $userrole == 'RU'||$userrole == 'OP')
           {
...
//     $wcond and
           ...
           else if ($userrole == 'SALES')
           {
               ...
               $sql = "select w.wonum, w.wotype, c.name, w.po_num,w.po_date,
                           w.status,w.condition, w.wo2type, emp.fname, emp.lname,
                           w.create_date, w.recnum, w.descr, u.initials,
                           date_format(w.sch_due_date,'%y-%m-%d') as sch_due_date,
                           date_format(w.actual_ship_date,'%y-%m-%d') as actual_ship_date,
                           date_format(w.book_date,'%d %b %y') as book_date,
                           date_format(w.revised_ship_date,'%d %b %y') as revised_ship_date,
                           reorder,b.bomnum ,b.recnum, w.qty, w.po_qty,w.original_qty
